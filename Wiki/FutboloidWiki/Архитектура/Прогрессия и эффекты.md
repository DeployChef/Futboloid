...
> [!note] Статус
> **GDD:** [[../GDD/09 Карточки перков и XP|Карточки перков и XP]] — зафиксированы цвета карт, level-up в матче, уровни перков. Здесь — **архитектура**; числа баланса — в SO.
...
При старте матча: `RunStateService` → модификаторы генерации врагов и/или `StatusEffectService.ApplyPassives(...)` — см. [[Генерация врагов#Кривая pacing (нелинейная сложность)]].
...
| **Перк (забег)** | `RunStateService` | **MVP:** level-up → **BonusPick** в матче, 1 из 3 карт; уровни 1–5 |
| **Карточка перка** | `PerkDefinition` SO | презентация + `PerkCardColor`; эффект по `Id` в коде |
| **Бафф (timed)** | `StatusEffectService` | трибуна, арбитр, разовые в матче — **не** карточки перков |
| **Дебафф (timed)** | `StatusEffectService` | как бафф, с таймером на HUD |
...
Состояние [[Машины состояний#Уровень 3: Pitch FSM (PitchStateMachine)|`BonusPick`]] — **единственный** экран выбора перков в MVP. Между матчами турнира отдельного пика перков **нет**.

GDD: [[../GDD/09 Карточки перков и XP]].

### Поток

1. `RunStateService.AddXp` → при level-up публикует `RunLevelUpEvent` (или callback в оркестратор).
2. `PitchStateMachine` → `BonusPick`; симуляция поля **на паузе** (`timeScale = 0` / не тикают мяч, враги, `StatusEffectService.Tick`).
3. `PerkPickService` (или метод в `RunStateService`) роллит **3** `PerkDefinition` из `PerkCatalog`.
4. `BonusPickController` показывает overlay: 3× prefab карточки (цвет фона по `PerkCardColor`, иконка и текст из SO).
5. Игрок выбрал → `RunStateService.ApplyPerkPick(id)`:
   - `perkLevels[id]++` (cap `MaxLevel`);
   - пересчёт модификаторов забега;
   - опционально `StatusEffectService` не нужен для перков забега — множители читают из `RunStateService` / агрегатора.
6. → `Simulating`, пауза снята.

```mermaid
stateDiagram-v2
    Simulating --> BonusPick: RunLevelUp
    BonusPick --> Simulating: PerkPicked
```

### Карточки: данные vs UI

| Что | Где |
|-----|-----|
| Рамка / layout карточки | **Один** prefab в Unity (ты) |
| Иконка, название, описание | `PerkDefinition` SO |
| Цвет (зелёный / жёлтый / красный) | поле `CardColor` → спрайт рамки или tint |
| Логика эффекта | код по `Id` (+ `ValuePerLevel` из SO для простых статов) |
| Каталог для ролла | `PerkCatalog` SO — список всех перков |

> [!important] Простое добавление перка
> SO (картинка + текст) + строка в каталоге + ветка в `ApplyPerkEffect`. Без generic-дерева и reflection на первом этапе.

### Уровни перка

`RunStateService` хранит `Dictionary<string, int> PerkLevels`.

Эффект уровня N: обычно `value = ValuePerLevel * N` или таблица в SO. При повторном попадании в ролл карточка показывает **следующий** уровень.

На max level перк **исключается** из пула ролла.

### Перки vs timed-эффекты

| | Перк забега | Timed бафф/дебафф |
|---|-------------|-------------------|
| Живёт | весь забег (и дальше в турнире) | секунды / до конца матча |
| Выдача | BonusPick | трибуна, арбитр, скилл |
| HUD | шкала XP + опционально иконки пассивов позже | кружочки с кольцом таймера |
| Хранение | `RunStateService` | `StatusEffectService` (Game scope) |

Цвет карточки — только SO + UI; логика эффекта — **отдельно в коде** для каждого `Id`. Не путать с timed-эффектами `StatusEffectService` (трибуна, арбитр).

### События (шина, черновик)
...
public readonly struct RunLevelUpEvent
{
    public int NewRunLevel;
}

public readonly struct BonusPickOfferedEvent
{
    public PerkDefinition Offer0;
    public PerkDefinition Offer1;
    public PerkDefinition Offer2;
    public int LevelIfPicked0; // текущий+1 для UI
    // ...
}
...
См. [[UI и оверлеи]], [[Шина событий]], [[MatchFlow и таймер#Принцип таймера (как у баффов в §7)|таймер матча — тот же паттерн]].
...
│   └── IPlayerProgressionService.cs
├── Run/                            # MVP
│   ├── RunStateService.cs          # XP, run level, PerkLevels, модификаторы
│   ├── RunProgressionSettings.cs   # SO: xp hit/kill, кривая level-up
│   ├── PerkDefinition.cs           # SO: id, icon, texts, color, maxLevel
│   ├── PerkCatalog.cs              # SO: список перков для ролла
│   └── PerkCardColor.cs            # enum Green, Yellow, Red
└── StatusEffects/                  # timed в матче, не карточки
...
- [x] Когда выбор карт → **во время матча**, `BonusPick`, пауза
- [x] Перки стакаются → **уровни 1–5** на id внутри забега
- [x] Цвет карточки → поле в `PerkDefinition` SO (Green / Yellow / Red), только UI
- [x] Механики перков → по одному в коде, без правил «по цвету»
- [ ] Числа XP: hit vs kill, кривая level-up
- [ ] Дебаффы с трибун — `StatusEffectService`, не карточки
- [ ] Сохранение середины турнира в MVP
- [ ] Post-MVP: мета-дерево, Hall of Fame
...
- [[Машины состояний]]
- [[DI и LifetimeScope]]
- [[../GDD/09 Карточки перков и XP|GDD: карточки перков]]
