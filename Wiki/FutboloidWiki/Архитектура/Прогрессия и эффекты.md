...
| **Карточка перка** | `PerkDefinition` SO | `Icon` + `CardFrame` + тексты; эффект по `Id` в коде |
...
4. `BonusPickController` показывает overlay: 3× prefab карточки (`CardFrame`, `Icon`, текст из SO).
...
- [x] `CardFrame` + `Icon` в `PerkDefinition` SO
