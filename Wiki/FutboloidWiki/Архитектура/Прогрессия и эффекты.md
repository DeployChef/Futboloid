...
| **Карточка перка** | `PerkDefinition` SO | `CardFrame` + тексты; эффект по `Id` в коде |
...
4. `BonusPickController` показывает overlay: 3× prefab карточки (`CardFrame`, текст из SO).
...
| Card frame, название, описание | `PerkDefinition` SO |
...
- [x] Card Frame → поле `CardFrame` в `PerkDefinition` SO
