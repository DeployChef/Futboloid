...
← [[Движение мяча]] | [[Индекс архитектуры]]
...
| Ворота | `BoxCollider2D` | не нужен | **да** |
| Зона аут / game over | `BoxCollider2D` | не нужен | **да** |
| Мяч | опционально `CircleCollider2D` | **нет** (вариант A) или **Kinematic** (вариант B) | только если B + триггеры |

**Слои:** `Wall`, `Keeper`, `Defender`, `Goal` — собираются в `BallSettings.contactMask` для cast.
