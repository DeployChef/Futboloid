...
| Состояние | UI (Root overlay) | Симуляция поля | Ввод игрока | Match HUD (Game) |
|-----------|-------------------|----------------|-------------|------------------|
| `MainMenu` | Главное меню + **лидерборд** (поверх поля) | Боты играют сами | Нет (или только UI) | **Да** — HUD на Game, под меню |
| `OnField` | Нет Root-оверлея (или минимум) | Полная / по Pitch FSM | Да | **Да** |
| `Tournament` | Турнирная сетка | Поле на паузе или затемнено | Только UI сетки | **Нет** |
| `Pause` | Escape-меню (MVP: тот же MainMenu + Continue) | `timeScale = 0` | Только UI паузы | **Да** — меню поверх поля и HUD |

> [!important] Match HUD ≠ Root overlay
> HUD — **часть сцены Game** (таймер, счёт). Главное меню и пауза — **оверлеи Root** поверх поля; HUD **не скрываем** при `MainMenu` / `Pause`. Скрываем только при уходе с поля (`Tournament` и аналоги). См. [[UI и оверлеи#Match HUD]], [[MatchFlow и таймер#HUD: слайдер на сцене Game]].
