...
```text
Defender (prefab) — root
├── DefenderView           ← движение, reshuffle, контакт с мячом
├── DefenderHealth         ← HP, урон, смерть, heal при reshuffle
├── DefenderViewGizmos     ← editor gizmos (полевые круги; GK-парабола на GoalAnchor)
├── BoxCollider2D          ← один на всё время жизни
└── Visual
    ├── CharacterAnimationPresenter   ← Idle/Run + flip по velocityX
    └── Animator
```

| Что | Как |
|-----|-----|
| Коллайдер | **Один** `Collider2D`, layer `Defender`, ссылка в `DefenderView` |
| Анимация | `CharacterAnimationPresenter` на `Visual`; bool `Run`, flip `localScale.x` |
| HP | `DefenderHealth` на root; `DefenderView` дергает `ApplyDamage` / `Heal` |
| Режим | `DefenderRole`: `Field` \| `Goalkeeper` |
| Движение | `DefenderLogic` (pure C#, transient DI) + `DefenderMoveResult` |
| GK-парабола | `GoalAnchor` на сцене — **DI**, не SerializeField на prefab |
...
- Параметры зоны ворот — компонент **`GoalAnchor`** на сцене (`RegisterComponentInScene` + inject в `DefenderView`, spawner, promotion).
- `t = clamp((ballX − centerX) / halfWidth, −1, 1)` — цель слежения по X мяча.
- `trackSpeed` — `MoveTowards` текущего `t` к цели; позиция через `GoalAnchor.PositionOnParabola(t)`.
- `BallView` — через DI в `DefenderView`, не `Find`.
...
| `ChaseBall` | к мячу по всему полю; без мяча — wander |
...
`DefenderGridRegistry` — только **реестр** заспавненных `DefenderView` (`AliveCount`, вайп, ближайший для паса). Параметры promotion / reshuffle — в **`DefenderMatchSettings`** SO (DI из App scope).

### Reshuffle (после гола)

> **Статус кода:** ✅ `DefenderReshuffleService` + DOTween на живых, мяч на `BallKickoffAnchor`, игрок-GK в центр.

| Шаг | Кто |
|-----|-----|
| `GoalScored` → `Reshuffle` | `PitchStateMachine` |
| Мяч: анимация на `BallKickoffAnchor` | `BallView` |
| Живые: +25% max HP, tween на home / ворота | `DefenderView` + `DefenderHealth` |
| Длительность / arrive | `DefenderMatchSettings` |
...
│   ├── DefenderView.cs
│   ├── DefenderHealth.cs
│   ├── DefenderViewGizmos.cs
│   ├── DefenderLogic.cs
│   ├── DefenderMoveResult.cs
│   ├── DefenderReshuffleMotion.cs
│   ├── DefenderMatchSettings.cs      # SO: promotion + reshuffle тайминги
│   ├── Anchors/GoalAnchor.cs
│   ├── Services/
│   │   ├── DefenderGridRegistry.cs
│   │   ├── DefenderSpawner.cs
│   │   ├── DefenderPromotionService.cs
│   │   └── DefenderReshuffleService.cs
│   └── Generation/ …
├── Keeper/
│   ├── GoalkeeperView.cs
│   ├── GoalkeeperMotor.cs
│   └── GoalkeeperReshuffleMotion.cs
```
...
- [ ] Prefab `Defender`: `DefenderView`, `DefenderHealth`, `DefenderViewGizmos`, один `Collider2D`, layer `Defender`
- [ ] `Visual`: `Animator` + `CharacterAnimationPresenter` → ссылка в `DefenderView`
- [ ] `DefenderHealth`: `Hp Label`, ссылка **Defender** → root `DefenderView`
- [ ] `DefenderViewGizmos`: **Defender**, **Health**, **Body Collider**
- [ ] `Defenders/GoalAnchor` — компонент **`GoalAnchor`** (gizmo параболы здесь)
- [ ] `Defenders/DefenderSlots` — **`DefenderSlotLayout`**, сетка 5×7
- [ ] `Defenders/Spawned` — пустой Transform, ссылка в **`DefenderSpawner`**
- [ ] **`DefenderSpawner`**: prefab, Slot Layout, Spawn Root (Goal Anchor **не** нужен — DI)
- [ ] `DefenderGridRegistry` на `Defenders` (без таймингов promotion)
- [ ] **Нет** ручных `Defender_*` на сцене
