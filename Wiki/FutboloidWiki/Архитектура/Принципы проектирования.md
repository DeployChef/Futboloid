...
| **Шина** | Связь с HUD, FSM, аналитикой | `IGameEventBus` (App scope, `Futboloid.Core.Bus`) |
...
```csharp
// DefenderView — футболист соперника (поле или вратарь — один prefab, DefenderRole)
public class DefenderView : MonoBehaviour
{
    ...
    public void SetRole(DefenderRole newRole) { role = newRole; /* visual + motor */ }

    void OnBallContact(BallMotion ball) { /* hitBehavior, HP, bus */ }
}
```
...
| **`DefenderMotor`** (позже) | парабола GK + field patrol — если view разрастётся | то же |
...
| HP / роль футболиста соперника? | `DefenderView` (`Field` / `Goalkeeper`) |
...
- MonoBehaviour — `[Inject]` при старте Game scope (VContainer)
- **Нет** `GameManager.Instance`
- **Нет** прямого `Time.timeScale` — только [[PauseCoordinator]]
