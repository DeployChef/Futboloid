...
│  └─ GameDirector                            │
├─────────────────────────────────────────────┤
│  GameScene (additive, всегда после boot)    │
│  ├─ Поле, ворота, мяч                       │
│  ├─ Вратарь игрока                          │
...
| # | Сцена | Назначение |
|---|-------|------------|
| 0 | `Bootstrap.unity` | **Единственная** стартовая сцена в билде |
...
Минимальная сцена. Один объект с компонентом `Bootstrap`:
...
Дублирование защиты: `[RuntimeInitializeOnLoadMethod]` + `Bootstrap.Awake` с guard «уже инициализировано».
